...
<Button
	variant="outline"
	class="relative w-full justify-between bg-transparent text-sm text-muted-foreground md:w-40 lg:w-64"
	on:click={() => (open = true)}
	{...$$restProps}
>
		...
		{#if $page.data.session}
			...
			<Command.Group heading="Challenges">
				{#each $page.data.challenges as challenge}
					<Command.Item
						value={`${challenge.name} ${challenge.category.name} ticket`}
					...
					<Command.Item
						value={`${challenge.name} ${challenge.category.name} focus`}
